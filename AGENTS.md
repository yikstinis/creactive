...
- Each case's screenshotted element is a fixed-padding (80px) container with a neutral background, wrapping the prop-under-test's own box, itself wrapping 3 fixed 32×32 reference squares (colors distinct from the container's background, so all 3 stay visible) — the squares give a constant-size reference so a prop's effect (e.g. padding) reads visually against something that never changes, and safe-area insets are added on top of the fixed padding (`useSafeAreaInsets` from `react-native-safe-area-context`) so the frame isn't affected by device notches/home indicators. If cases no longer fit one viewport, host them in a `ScrollView` (testID'd) — Playwright's `toHaveScreenshot()` scrolls a locator into view on its own, but Detox needs `waitFor(element(by.id(caseTestID))).toBeVisible().whileElement(by.id(scrollViewTestID)).scroll(300, 'down')` before each screenshot.
