...
- Each case's screenshotted element is a fixed-padding (80px) container in one color, wrapping the prop-under-test's own box in a contrasting color, itself wrapping 3 fixed 32×32 reference squares (colors distinct from both, so all 3 stay visible) — the squares give a constant-size reference and the contrasting fill makes the prop's own box (e.g. its padding) a visibly distinct band, not just blank space indistinguishable from the fixed frame around it. Skip `react-native-safe-area-context`/`useSafeAreaInsets` here even though this is a full-bleed screen — real notches aren't the point of these fixtures, and inset listeners underneath Detox's Android scroll gesture were the suspected cause of an Espresso "window hasn't had focus" hang (an infinite-relayout symptom) once cases stopped fitting in one viewport. If cases no longer fit one viewport, host them in a `ScrollView` (testID'd) — Playwright's `toHaveScreenshot()` scrolls a locator into view on its own, but Detox needs `waitFor(element(by.id(caseTestID))).toBeVisible().whileElement(by.id(scrollViewTestID)).scroll(300, 'down')` before each screenshot.
