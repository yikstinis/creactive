...
- Baselines are committed PNGs, kept in a `snapshots/{platform}-{device}/` folder next to each component's visual test files (e.g. `snapshots/linux-chromium/`, `snapshots/android-pixel-7/`, `snapshots/ios-iphone-17/`), named to match the `snapshot-test-{platform}-{device}` job naming in maintain.yml. Regenerate them on CI, not locally — cross-machine font/anti-aliasing differences cause false-positive diffs otherwise.
